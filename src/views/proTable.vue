  ...
  <div class="content-main">
    ...
    <div>
      <el-table :data="tableData" border class="table" ref="multipleTable" header-cell-class-name="table-header">
        ...
        <el-table-column prop="createTime" label="最新发版时间">
          <template #default="scope">
            <span>{{ scope.row?.lastPublishRecord?.createTime || "-" }}</span>
          </template>
        </el-table-column>
        <el-table-column prop="version" label="最新版本" width="100">
          <template #default="scope">
            <span>{{ scope.row?.lastPublishRecord?.version || "-" }}</span>
          </template>
        </el-table-column>
        <el-table-column prop="projectName" label="最新发版人"><template #default="scope">
            <span>{{ scope.row?.lastPublishRecord?.createUserName || "-" }}</span>
          </template></el-table-column>
        <el-table-column label="操作" width="350">
          <template #default="scope">
            <el-button text :icon="Edit" type="primary" @click="handleEdit(scope.$index, scope.row)">
              去发版
            </el-button>
            <el-button text :icon="Document" @click="handleRecord(scope.row)">
              发版记录
            </el-button>
